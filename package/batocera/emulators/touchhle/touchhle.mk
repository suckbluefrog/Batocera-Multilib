...
define TOUCHHLE_INSTALL_TARGET_CMDS
	$(INSTALL) -D -m 0755 $(@D)/$(TOUCHHLE_BIN_DIR)/touchHLE \
		$(TARGET_DIR)/usr/bin/touchHLE
	mkdir -p $(TARGET_DIR)/usr/share/touchhle/touchHLE_apps
	rm -rf $(TARGET_DIR)/usr/share/touchhle/touchHLE_dylibs \
		$(TARGET_DIR)/usr/share/touchhle/touchHLE_fonts
	cp -a $(@D)/touchHLE_dylibs $(TARGET_DIR)/usr/share/touchhle/
	cp -a $(@D)/touchHLE_fonts $(TARGET_DIR)/usr/share/touchhle/
	$(INSTALL) -D -m 0644 $(@D)/touchHLE_default_options.txt \
		$(TARGET_DIR)/usr/share/touchhle/touchHLE_default_options.txt
	$(INSTALL) -D -m 0644 $(@D)/touchHLE_options.txt \
		$(TARGET_DIR)/usr/share/touchhle/touchHLE_options.txt
	$(INSTALL) -D -m 0644 $(@D)/OPTIONS_HELP.txt \
		$(TARGET_DIR)/usr/share/touchhle/OPTIONS_HELP.txt
	$(INSTALL) -D -m 0644 $(@D)/touchHLE_apps/README.txt \
		$(TARGET_DIR)/usr/share/touchhle/touchHLE_apps/README.txt
endef
