...
class _Libxml2API(_XMLBase):
    def __init__(self, xml):
        ...
        keep_blanks = (
            getattr(libxml2, "keepBlanksDefault", None)
            or getattr(libxml2, "keepdefaultsblank", None)
        )
        if keep_blanks:
            keep_blanks(1)
...
class _LxmlAPI(_XMLBase):
    def __init__(self, xml):
        _XMLBase.__init__(self)
        if ET is None:
            raise RuntimeError("lxml.etree is required when libxml2.parseDoc is unavailable")
        parser = ET.XMLParser(remove_blank_text=False)
        self._doc = ET.fromstring(xml.encode("utf-8"), parser=parser)
    ...
    def copy_api(self):
        return _LxmlAPI(self._node_tostring(self._doc))

    def _xpath_eval(self, xpath):
        return self._doc.xpath(xpath, namespaces=self.NAMESPACES)

    def _find(self, fullxpath):
        xpath = _XPath(fullxpath).xpath
        try:
            nodes = self._xpath_eval(xpath)
        except Exception as e:
            log.debug("fullxpath=%s xpath=%s eval failed", fullxpath, xpath, exc_info=True)
            raise RuntimeError("%s %s" % (fullxpath, str(e))) from None
        if not nodes:
    ...
    def count(self, xpath):
        return len(self._xpath_eval(xpath))
    ...
    def _node_new(self, xpathseg, parentnode):
        ignore = parentnode
        if not xpathseg.nsname:
            return ET.Element(xpathseg.nodename)
        return ET.Element("{%s}%s" % (self.NAMESPACES[xpathseg.nsname], xpathseg.nodename))
    ...
    def _node_has_content(self, node):
        return bool(list(node) or node.attrib)
...
if hasattr(libxml2, "parseDoc"):
    XMLAPI = _Libxml2API
elif ET is not None:
    XMLAPI = _LxmlAPI
else:  # pragma: no cover
    raise RuntimeError("No usable XML backend: missing libxml2.parseDoc and lxml.etree")
