...
def trigger_android_rom_refresh():
    global refresh_timer
    update_script = "/usr/bin/batocera-waydroid-update"
    ...
    def _run_update():
        ...
        try:
            ...
            logging.warning("Failed to refresh Android ES entries: %s", exc)

    if refresh_timer is not None:
        refresh_timer.cancel()
    refresh_timer = threading.Timer(2.0, _run_update)
...
def start(args, session, unlocked_cb=None):
    ...
    def updateDesktopFile(appInfo):
        ...
        desktop_file.set_string("Desktop Entry", "Exec", f"/usr/bin/batocera-waydroid-app launch {packageName}")
        ...
        desktop_file.set_string("Desktop Action app-settings", "Exec", f"/usr/bin/batocera-waydroid-app intent android.settings.APPLICATION_DETAILS_SETTINGS package:{packageName}")
