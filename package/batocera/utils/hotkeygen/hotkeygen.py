...
GUSERCOMMONCONTEXT_GLOB: Final = "common_context*.conf"
...
def get_common_context_keys() -> dict[str, int|str]:
    ...
    if GUSER_DIR.exists():
        for user_context_file in sorted(GUSER_DIR.glob(GUSERCOMMONCONTEXT_GLOB)):
            with user_context_file.open() as file:
                data = json.load(file)
                keys |= load_context_keys(data)
    return keys
