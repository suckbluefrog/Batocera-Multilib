...
def _apply_arcade_analog_stick_preference(mappings: dict[str, str], system: Emulator) -> None:
    if system.config.get('arcade_analog_device') != 'left_stick':
        return

    # Keep arcade analog controls on the left stick when requested.
    mappings["JOYSTICKRIGHT_UP"] = "joystick1up"
    mappings["JOYSTICKRIGHT_DOWN"] = "joystick1down"
    mappings["JOYSTICKRIGHT_LEFT"] = "joystick1left"
    mappings["JOYSTICKRIGHT_RIGHT"] = "joystick1right"
...
def generatePadsConfig(cfgPath: Path, playersControllers: Controllers, sysName: str, altButtons: MameControlScheme, customCfg: bool, specialController: str, decorations: str | None, useGuns: bool, guns: Guns, useWheels: bool, wheels: DeviceInfoMapping, useMouse: bool, multiMouse: bool, system: Emulator) -> None:
    ...
    _apply_arcade_analog_stick_preference(mappings, system)
